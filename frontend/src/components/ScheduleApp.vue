<template>
        ...
        <div v-if="mode === 'view'" class="appointment-cards">
          ...
          <div v-for="appointment in appointments" :key="appointment.id" class="appointment-card">
            ...
            <div class="appointment-actions">
              <button v-if="appointment.status === 'pending'" @click.stop="selectAppointment(appointment)" class="update-button">Update</button>
              <button v-if="appointment.status === 'pending'" @click.stop="cancelAppointment(appointment.id)" class="cancel-button">Cancel</button>
            </div>
          </div>
        </div>
  ...
  import { collection, getDocs, updateDoc, doc, addDoc, setDoc, query, where } from 'firebase/firestore';
  ...
  const updateAppointment = async () => {
    if (selectedAppointment.value && user.value) {
      try {
  ...
  const scheduleAppointment = async () => {
    ...
    try {
      ...
      console.log('Appointment scheduled successfully. Document ID:', docRef.id);
      alert('Appointment scheduled successfully! Status: Pending');
      setMode('view');
  ...
  const fetchAppointments = async () => {
    ...
    try {
      console.log('Fetching appointments for user:', user.value.email);
      const q = query(collection(db, 'appointments'), where("userEmail", "==", user.value.email));
