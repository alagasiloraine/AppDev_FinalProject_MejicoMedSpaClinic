<template>
  <div class="container">
    <div class="header">
      <h1>Services and Treatments List</h1>
    </div>

    <div class="lists-container">
      <div v-if="loadingServices" class="loading">Loading services...</div>
      <div v-else class="list-section" v-for="service in services" :key="service.id">
        <li class="list-item">
          <h3>{{ service.name }}</h3>
          <div v-if="loadingTreatments" class="loading">Loading treatments...</div>
          <ul v-else class="item-list">
            <li
              v-for="treatment in filteredTreatments(service.name)"
              :key="treatment.name"
              class="list-item"
            >
          ...
          <div v-if="!loadingTreatments && filteredTreatments(service.name).length === 0">
...
// Function to filter treatments based on the service name
const filteredTreatments = (services) => {
  return treatments.value.filter(treatment => treatment.services === services);
};
