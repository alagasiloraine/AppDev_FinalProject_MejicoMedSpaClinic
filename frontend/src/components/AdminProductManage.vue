    ...
    <div class="bg-white p-4 rounded-lg shadow mb-8">
      ...
      <form @submit.prevent="saveProduct" class="space-y-4">
        <div>
          ...
          <input v-model="currentProduct.name" id="name" type="text" required
        ...
        <div>
          ...
          <input v-model="currentProduct.price" id="price" type="number" step="0.01" required
            class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-purple-500 focus:ring focus:ring-purple-500 focus:ring-opacity-50">
        </div>
        <div>
          <label for="quantity" class="block text-sm font-medium text-gray-700">Quantity</label>
          <input v-model="currentProduct.quantity" id="quantity" type="number" required
            class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-purple-500 focus:ring focus:ring-purple-500 focus:ring-opacity-50">
        </div>
        <div>
          <label for="treatmentServices">Select Category</label>
        ...
        <div>
          ...
          <button v-if="editingProduct" @click="cancelEdit" type="button"
            class="ml-2 bg-gray-300 text-gray-800 px-4 py-2 rounded-md hover:bg-gray-400 focus:outline-none focus:ring-2 focus:ring-gray-500 focus:ring-offset-2">
            Cancel
          </button>
        </div>
      </form>
...
const saveProduct = async () => {
  try {
    ...
    if (editingProduct.value) {
      await updateDoc(doc(database, 'products', editingProduct.value), productData);
    } else {
