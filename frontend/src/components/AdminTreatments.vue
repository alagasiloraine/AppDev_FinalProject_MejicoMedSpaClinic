...
const getServiceNameById = (serviceId) => {
  const service = services.value.find(s => s.name === serviceId);
...
const addTreatment = async () => {
  try {
      const treatmentRef = await addDoc(collection(database, 'treatments'), {
          ...currentTreatment.value,
...
const updateTreatment = async () => {
  try {
      const treatmentRef = doc(database, 'treatments', editingTreatment.value.id);
...
const deleteTreatment = async (id) => {
  if (confirm('Are you sure you want to delete this treatment?')) {
      try {
          await deleteDoc(doc(database, 'treatments', id));
